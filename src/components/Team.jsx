...
function Team({ team }) {
  ...
  useEffect(() => {
    ...
    const updateAvailablePlayers = (all, team) => {
      ...
      const allPlayersArray = Object.entries(all).map(([key, player]) => ({
        key,
        ...player,
      }));

      const filteredPlayers = allPlayersArray.filter(
        (player) => !teamIds.has(player.key)
      );
  ...
  const handleClear = async (playerKey) => {
    ...
    try {
      ...
      await update(teamRef, {
        spentAmount: Math.max(0, currentSpent - player.playerValue),
      });
  ...
  const calculateTeamPoints = (players) => {
    ...
    players.forEach((player) => {
      const battingStrike = calculateBattingStrikeRate(
        player.totalRuns,
        player.ballsFaced
      );
      const battingAverage = calculateBattingAverage(
        player.totalRuns,
        player.inningsPlayed
      );
      const bowlingStrikeRate = calculateBowlingStrikeRate(
        player.oversBowled,
        player.wickets
      );
  ...
  const filteredPlayers = availablePlayers.filter(
    (player) => selectedCategory === "" || player.category === selectedCategory
  );
  ...
  return (
    <div className="">
      ...
      <SelectPlayersTable
        onAddPlayer={(id) => handleAddPlayer(id)}
